...
const CASE_GROUPS = [
  {
    ...
    fallbackText: "Multi-shot Echo-LongVideo output preview.",
  },
  {
    key: "short",
    selector: "#short-video [data-case-track]",
    fallbackText: "Short Echo-LongVideo output preview.",
  },
];
const carouselStates = new WeakMap();
const CASE_PREVIEW_VOLUME = 0.86;
const GALLERY_SECTION_IDS = new Set(["long-video", "short-video"]);

function normalizeNavHrefForSection(sectionId) {
  return GALLERY_SECTION_IDS.has(sectionId) ? "#long-video" : `#${sectionId}`;
}
...
function seekCaseVideo(video, progress) {
  ...
  const percent = Number.parseFloat(progress.value) || 0;
  video.currentTime = (Math.min(100, Math.max(0, percent)) / 100) * duration;
...
function enableCaseAudioHover() {
  document.querySelectorAll(".case-card").forEach((card) => {
    if (card.dataset.audioHoverReady === "true") {
      return;
    }

    card.dataset.audioHoverReady = "true";
    card.addEventListener("mouseenter", () => {
      const video = card.querySelector("video");
      if (video) {
        playCaseVideo(video, { restart: true, audible: true });
      }
    });
